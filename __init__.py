...
def create_app():
    ...
    app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'dev-key-69-420')    # Setup database connection
    ...
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False    # Initialize SQLAlchemy with this app
    db.init_app(app)    # Import models before creating database tables
    from .models import User    # Create the database tables
    ...
    login_manager = LoginManager()    # If a user tries to access something requiring login redirect them to the auth.login route
    login_manager.login_view = 'auth.login' # type: ignore[attr-defined]
    # Attach LoginManager to the app.
    login_manager.init_app(app)    # Define how Flask-Login loads a user from the database using the user ID stored in their session
    ...
    app.register_blueprint(auth_blueprint)    # Import and register general routes
    from .main import main as main_blueprint
    app.register_blueprint(main_blueprint)    # Import and register chat routes
    from .chat import chat as chat_blueprint
    app.register_blueprint(chat_blueprint)    # Return the fully configured Flask app
    return app
